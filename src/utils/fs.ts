...
import { CONFIG_FILE_NAME, INDEX_FILE_NAME, TABS_DIR_NAME } from '../constants.js';
 ...
 */
export async function copyDocumentationFiles(docsDir: string, themeDir: string): Promise<void> {
  ...
  if (fs.existsSync(indexSrc)) {
    ...
    logWarning('index.md not found in docs directory. Using default index from theme.');
  }
  ...
  const files = (await globber.glob()).filter(f => !f.endsWith(INDEX_FILE_NAME));
